...
1. `tutorial.html` 상단의 제목이 이제 오래되었으므로,
   파일을 재빌드해야 합니다.

2. `index.html`의 목차에 아직 이전 제목이 있으므로,
   해당 문서를 재빌드해야 합니다.

3. `api.html`의 첫 번째 문단에 포함된 상호 참조에
   아직 이전 챕터 제목이 있으므로,
   이것도 재빌드해야 합니다.
Sphinx는 무엇을 할까요? 
...
\aosafigure[180pt]{contingent-images/figure1.png}{세 개의 입력 텍스트를 파싱하여 생성된 세 개의 파일.}{500l.contingent.graph}
...
\aosafigure[240pt]{contingent-images/figure2.png}{언급된 제목이 변경될 때마다 `index.html`을 재빌드할 준비.}{500l.contingent.graph2}
...
문서를 파싱하고 변환하는 것,
그리고 렌더링하는 것까지:
\aosafigure[240pt]{contingent-images/figure3.png}{작업 그래프.}{500l.contingent.graph3}
...
6. 작업의 반환 값을 반환하여,
   이 래퍼의 호출자가 단순히 일반 작업 함수 자체를
   호출한 것이 아님을 알 수 없게 합니다.
...
\aosafigure[240pt]{contingent-images/figure4.png}{입력 파일과 HTML 출력 간의 완전한 관계 집합.}{500l.contingent.graph4}
